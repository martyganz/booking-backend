...
const getPassangers= (args) => {
  if(args.id){
    ...
    return klmMock.filter(passanger => passanger.passengers.id===passangerId).map(item=>item.passengers)[0];

  }else{
    // TODO return empty array when there is no records
    return [];
  }
}
...
const getRetrieveBooking= (args)=>{
  if(args.bookingCode && args.lastName){
    ...
    const filteredData= klmMock.filter(details=>details.bookingCode === bookingCode && details.passengers.lastName===passangerLastName)[0]
